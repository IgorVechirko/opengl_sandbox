...
namespace GLSandbox
{
	const char* positionNormalLightProp_frag = R"(
		#version 330 core
		...
		{
			vec3 resultColor = u_directLight.ambient * u_material.ambient;

			vec3 lightDir = normalize( -u_directLight.direction );
			float intensive = max( dot(normal, lightDir), 0.0f );
			resultColor += ( intensive * u_material.diffuse ) * u_directLight.diffuse;

			vec3 reflectDir = reflect( -lightDir, normal );
			float spec = pow( max(dot(viewDir, reflectDir), 0.0f) , u_material.shininess );
			resultColor += ( spec * u_material.specular ) * u_directLight.specular;
		...
		{
			vec3 resultColor = lightProperties.ambient * u_material.ambient;

			vec3 lightDir = normalize( lightProperties.pos - f_pos );
			float intensive = max( dot(normal, lightDir), 0.0f );
			resultColor += ( intensive * u_material.diffuse ) * lightProperties.diffuse;

			vec3 reflectDir = reflect( -lightDir, normal );
			float spec = pow( max(dot(viewDir, reflectDir), 0.0f) , u_material.shininess );
			resultColor += ( spec * u_material.specular )  * lightProperties.specular;
